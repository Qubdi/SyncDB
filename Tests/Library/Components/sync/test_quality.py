...
class QualityExpectationTests(unittest.TestCase):
    ...
    def _connector_with_rows(self, rows, columns):
    ...
    def test_no_expectations_is_noop(self):
        connector = self._connector_with_rows([{"id": 1, "email": "a", "score": 5}], self.columns)
        result = _result()
    ...
    def test_min_rows_pass_and_fail(self):
        connector = self._connector_with_rows(
    ...
    def test_unsafe_column_name_rejected(self):
        connector = self._connector_with_rows([{"id": 1, "email": "a", "score": 1}], self.columns)
        with self.assertRaises(ValueError):
